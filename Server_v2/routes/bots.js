 ...
 * Один запрос для ВСЕХ анкет программы.
 * Вместо 10 запросов/минуту → 1 запрос/минуту
 */
router.post('/heartbeat-batch', asyncHandler(async (req, res) => {
    const {
        machineId,      // ID программы-бота
        version,        // Версия приложения
        platform,       // Платформа (Windows/Mac/Linux)
        uptime,         // Секунды работы
        memoryUsage,    // Использование памяти MB
        globalMode,     // mail/chat
        sessionStats,   // Статистика сессии
        profiles        // Массив анкет: [{ profileId, status, mode }]
    } = req.body;

    if (!machineId || !profiles || !Array.isArray(profiles)) {
        return res.status(400).json({
            success: false,
            error: 'machineId и profiles обязательны'
        });
    }
    ...
    console.log(`📦 Batch Heartbeat: ${machineId}, анкет: ${profiles.length}, IP: ${clientIp}`);

    // Результаты для каждой анкеты
    const results = [];

    // Обрабатываем каждую анкету
    for (const profile of profiles) {
        const { profileId, status = 'online', mode = 'idle' } = profile;
        ...
        try {
            ...
            if (!paymentStatus.isPaid) {
                results.push({
                    profileId,
                    status: paymentStatus.canTrial ? 'trial_available' : 'payment_required',
                    canTrial: paymentStatus.canTrial
                });
                continue;
            }

            // 2. Записываем heartbeat
            await pool.query(`
                INSERT INTO heartbeats (bot_id, account_display_id, status, ip, version, platform, timestamp)
                VALUES ($1, $2, $3, $4, $5, $6, NOW())
            `, [machineId, profileId, status, clientIp, version, platform]);

            // 3. Автосоздание анкеты если нет
            const exists = await pool.query(
                `SELECT 1 FROM allowed_profiles WHERE profile_id = $1`, [profileId]
            );
            ...
            results.push({
                ...
                status: 'ok',
                commands: {
                    mailingEnabled: !settings.rows[0]?.paused,
                    proxy: settings.rows[0]?.proxy || null
                }
            });

        } catch (err) {
            console.error(`❌ Batch heartbeat ошибка для ${profileId}:`, err.message);
            results.push({
                profileId,
                status: 'error',
                error: err.message
    ...
    // Обновляем данные программы-бота
    const extendedData = {
        uptime: uptime || 0,
        memoryUsage: memoryUsage || null,
        profilesTotal: profiles.length,
        profilesRunning: profiles.filter(p => p.mode !== 'idle').length,
        profilesStopped: profiles.filter(p => p.mode === 'idle').length,
    ...
    // Upsert бота
    const existsBot = await pool.query(`SELECT 1 FROM bots WHERE bot_id = $1`, [machineId]);
    if (existsBot.rows.length === 0) {
        await pool.query(
            `INSERT INTO bots (bot_id, platform, ip, version, status, last_heartbeat, extended_data)
             VALUES ($1, $2, $3, $4, 'online', NOW(), $5)`,
            [machineId, platform, clientIp, version, JSON.stringify(extendedData)]
        );
    } else {
        await pool.query(
            `UPDATE bots SET platform = COALESCE($1, platform), ip = COALESCE($2, ip),
             version = COALESCE($3, version), status = 'online', last_heartbeat = NOW(),
             extended_data = $4 WHERE bot_id = $5`,
            [platform, clientIp, version, JSON.stringify(extendedData), machineId]
        );
    }

    // Проверяем panic mode
    let panicMode = false;
    let stopSpam = false;
    try {
        const controlResult = await pool.query(`
            ...
            FROM user_settings
        `);
        panicMode = controlResult.rows[0]?.panic_mode === true;
        stopSpam = controlResult.rows[0]?.stop_spam === true;
    } catch (e) { /* ignore */ }

    console.log(`✅ Batch Heartbeat обработан: ${results.filter(r => r.status === 'ok').length}/${profiles.length} анкет`);
