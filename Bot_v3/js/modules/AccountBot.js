class AccountBot {
    ...
    createWebview() {
        // WebView теперь работает ЧЕРЕЗ ПРОКСИ (если настроен в proxy1-proxy6)
        // Прокси устанавливается в setWebviewProxy() ПЕРЕД вызовом createWebview()
        // Это решает проблему медленной загрузки при блокировке реального IP
        console.log(`[WebView] 🔧 Создание WebView для ${this.id} (С ПРОКСИ если настроен)...`);

        const webview = document.createElement('webview');
        webview.id = `webview-${this.id}`;
        // Используем отдельную сессию с прокси для каждого WebView
        // Прокси устанавливается через IPC 'set-webview-proxy' в setWebviewProxy()
        ...
        console.log(`[WebView] 📦 Partition: persist:wv_${this.id} (С ПРОКСИ), src: ${webview.src}`);
    ...
    // НАГРУЗКА: 1 запрос к LadaDate на каждого VIP каждые 45-75 сек
    // ПРИМЕЧАНИЕ: Запросы идут на LadaDate, НЕ на LABABOT сервер
    ...
    async loadProfileCard() {
        ...
        try {
            ...
            if (cached) {
                ...
                if (age < CACHE_DURATION) {
                    // Используем кэш
                    console.log(`[ProfileCard ${this.displayId}] Используем кэш (${Math.round(age/3600000)}ч назад)`);
                    this.applyProfileCard(data.photoUrl, data.nameText);
                    return;
                }
            }
        } catch(e) {
            console.log(`[ProfileCard ${this.displayId}] Ошибка чтения кэша:`, e.message);
        }

        // Кэша нет или устарел - загружаем через WebView
        ...
        try {
            ...
            // Сохраняем в кэш
            if (profileData.photoUrl || profileData.nameText) {
                localStorage.setItem(cacheKey, JSON.stringify({
                    photoUrl: profileData.photoUrl,
                    nameText: profileData.nameText,
                    timestamp: Date.now()
                }));
                console.log(`[ProfileCard ${this.displayId}] ✅ Сохранено в кэш`);
            }

            // Применяем данные
            this.applyProfileCard(profileData.photoUrl, profileData.nameText);
    ...
    applyProfileCard(photoUrl, nameText) {
    ...
    // НАГРУЗКА: 1 запрос к LadaDate каждые 3-7 сек на каждую анкету (через WebView)
    // ПРИМЕЧАНИЕ: Запросы идут на LadaDate, НЕ на LABABOT сервер
    async checkChatSync() {
    ...
    // НАГРУЗКА: 1 запрос к LadaDate каждые 20-35 сек на каждую анкету
    // ПРИМЕЧАНИЕ: Запросы идут на LadaDate, НЕ на LABABOT сервер
    async checkNewMails() {
        if (!this.token || !this.isMonitoring) return;
    ...
    // НАГРУЗКА: 2 запроса к LadaDate (сайт знакомств) каждую минуту на каждую анкету
    // ПРИМЕЧАНИЕ: Запросы идут на LadaDate, НЕ на LABABOT сервер
    startKeepAlive() {
        this.doActivity();
