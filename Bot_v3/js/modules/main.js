function createInterface(bot) {
    ...
    tab.innerHTML = `<span class="tab-id">${bot.displayId}</span><span class="tab-spinner"><i class="fa fa-sync fa-spin"></i></span><div class="status-dots"><div class="status-dot mail-dot"></div><div class="status-dot chat-dot"></div></div><span class="tab-close" onclick="closeTab(event, '${bot.id}')"><i class="fa fa-times"></i></span>`;
...
function renderBlacklist(botId) {
    ...
    // Отображаем в прямом порядке: старые сверху, новые снизу
    const displayData = [...data];

    displayData.forEach((id, index) => {
...
async function handleLoginOrUpdate() {
    ...
    if(editingBotId) {
        ...
        if(bot) { bot.login = l; bot.pass = p; bot.displayId = i; document.getElementById(`tab-${bot.id}`).innerHTML = `<div class="status-bar"><div class="status-bar-mail"></div><div class="status-bar-chat"></div></div><span class="tab-id">${i}</span><span class="tab-spinner"><i class="fa fa-sync fa-spin"></i></span><span class="tab-close" onclick="closeTab(event, '${bot.id}')"><i class="fa fa-times"></i></span>`; saveSession(); }
